export const sqlCommands = {
  sf: 'SELECT * FROM ??',
  sfw: 'SELECT * FROM ?? WHERE ?? = ?',
  sfww: 'SELECT * FROM ?? WHERE ?? = ? AND ?? = ?',
  sfwww: 'SELECT * FROM ?? WHERE ?? = ? AND ?? = ? AND ?? = ?',
  sfwi: 'SELECT * FROM ?? WHERE ?? IN (?)',
  sfwwi: 'SELECT * FROM ?? WHERE ?? = ? AND ?? IN (?)',
  dfw: 'DELETE FROM ?? WHERE ?? = ? ',
  dfww: 'DELETE FROM ?? WHERE ?? = ? AND ?? = ?',
  dfwww: 'DELETE FROM ?? WHERE ?? = ? AND ?? = ? AND ?? = ?',
  dfwi: 'DELETE FROM ?? WHERE ?? IN (?)',

  ii1: 'INSERT INTO ?? ( ?? ) VALUES (?)',
  ii2: 'INSERT INTO ?? ( ??, ?? ) VALUES (?, ?)',
  ii3: 'INSERT INTO ?? ( ??, ??, ??) VALUES (?, ?, ?)',
  ii4: 'INSERT INTO ?? ( ??, ??, ??, ??) VALUES (?, ?, ?, ?)',
  ii5: 'INSERT INTO ?? ( ??, ??, ??, ??, ??) VALUES (?, ?, ?, ?, ?)',

  im1: 'INSERT INTO ?? ( ?? ) VALUES ?',
  im2: 'INSERT INTO ?? ( ??, ?? ) VALUES ?',
  im3: 'INSERT INTO ?? ( ??, ??, ??) VALUES ?',
  im4: 'INSERT INTO ?? ( ??, ??, ??, ??) VALUES ?',
  im5: 'INSERT INTO ?? ( ??, ??, ??, ??, ??) VALUES ?',

  usw: 'UPDATE ?? SET ?? = ? WHERE ?? = ?',
  ussw: 'UPDATE ?? SET ?? = ? , ?? = ? WHERE ?? = ?',
  usww: 'UPDATE ?? SET ?? = ? WHERE ?? = ? AND ?? = ?',
  ussww: 'UPDATE ?? SET ?? = ? , ?? = ? WHERE ?? = ? AND ?? = ?',

  uswi: 'UPDATE ?? SET ?? = ? WHERE ?? IN (?)',
  uswwi: 'UPDATE ?? SET ?? = ? WHERE ?? = ? AND ?? IN (?)',
  usswi: 'UPDATE ?? SET ?? = ?, ?? = ? WHERE ?? IN (?)',

  insertVar(arr) {
    return arr.reduce((accum, item, index) => {
      if (index !== 1) {
        return `${accum} ,(?)`;
      }
      return accum;
    }, 'INSERT INTO ?? ( ?? ) VALUES (?)');
  },
};
